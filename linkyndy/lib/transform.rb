...
class Transform < Parslet::Transform
  rule(string: simple(:x)) { Ast::String.new(x) }
  rule(integer: simple(:x)) { Ast::Integer.new(x) }
  rule(boolean: simple(:x)) { Ast::Boolean.new(x) }
  ...
  rule(question: { string: simple(:string), type: simple(:type), identifier: simple(:identifier) }) { Ast::Question.new(Ast::String.new(string), Ast::Type.new(type), Ast::Identifier.new(identifier)) }
  rule(question: { string: simple(:string), type: simple(:type), identifier: simple(:identifier), value: subtree(:value) }) { Ast::Question.new(Ast::String.new(string), Ast::Type.new(type), Ast::Identifier.new(identifier), value) }
