...
module QL
  module Parser
    ...
    class Parser < Parslet::Parser
      ...
      rule(:expression) { sequence.as(:expression)}
      rule(:sequence) { boolean_expression }
      rule(:sequence_with_parenthesis) { str('(') >> sequence >> str(')') }
      ...
      rule(:multiplication_expression) { literal_or_variable_or_expression.as(:left) >> (multiplication_operator >> literal_or_variable_or_expression.as(:right)).repeat(1) | literal_or_variable_or_expression }
      rule(:literal_or_variable_or_expression) { negation? >> (boolean_literal | integer_literal | string_literal | variable | sequence_with_parenthesis) >> _ }
