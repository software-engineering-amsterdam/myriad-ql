  ...
  module Parser
    ...
    describe ExpressionTransformer do
      ...
      describe 'expressions' do
        context 'general' do
          it 'parses' do
            expect(form_transformer.apply(form_parser.expression.parse('5 + -10 > 0')).eval.value).to eq('false')
            expect(form_transformer.apply(form_parser.expression.parse('(5 + -10 > 0) == false')).eval.value).to eq('true')
          end
        end
