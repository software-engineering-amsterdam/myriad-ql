...
class FormChecker(model: FormModel) {
  ...
  private val checkExpressions: Issues =
    model.expressions.flatMap { case (expression, expectedType) => ExpressionChecker(model.identifiersWithType, expression, expectedType) }
