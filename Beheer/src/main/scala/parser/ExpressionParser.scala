...
trait ExpressionParser extends JavaTokenParsers {
  ...
  def expression: Parser[ExpressionNode] = infixOperationParser(comp, """\&\&|\|\|""".r) {
  ...
  private def factor: Parser[ExpressionNode] = (
    prefix
  ...
  private def date: Parser[DateLiteral] = """\d\d\d\d-\d\d-\d\d""".r ^^ (x => DateLiteral(dateFormat.parse(x)))
