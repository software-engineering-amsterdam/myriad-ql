...
public class CyclicDependenciesVisitor implements FormVisitor, ast.ExpressionVisitor<Type> {
    ...
    public void visit(ComputedQuestion question) {
        ...
        System.out.println("CYCLIC " + question.getVariable());
    ...
    public Type visit(IdExpression id) {
        // TODO add name to list
        System.out.println("CYCLIC " + id.getName());


        environment.addReference(current, id.getName());
        check();
    ...
    private void check() {
        List<String> references = environment.getReferences(current);

        for (String reference: references) {
            List<String> cycleReferences = environment.getReferences(reference);
            if (cycleReferences != null && cycleReferences.contains(current)) {
                // TODO add error!
